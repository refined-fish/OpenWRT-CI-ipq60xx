...
if [ -z "$output" ]; then
  case "$url" in
    *vernesong/mihomo*)
      cat <<'JSON'
{"assets":[{"name":"mihomo-linux-arm64-alpha-smart-new.gz","browser_download_url":"https://example.test/mihomo.gz"}]}
JSON
      ;;
    *EasyTier/EasyTier*)
      cat <<'JSON'
{"tag_name":"v9.9.9","assets":[{"name":"easytier-linux-aarch64-v9.9.9.zip","browser_download_url":"https://example.test/easytier.zip"}]}
JSON
      ;;
    *) printf '{}\n' ;;
  esac
...
with zipfile.ZipFile(sys.argv[1], "w") as z:
    for name in ("easytier-core", "easytier-cli", "easytier-web-embed"):
        z.writestr(f"easytier-linux-aarch64/{name}", name + "\n")
PY
    ;;
  *zashboard*)
    python3 - "$output" <<'PY'
...
PATH="$fake_bin:$PATH" OPENWRT_DIR="$openwrt_dir" WORKSPACE_DIR="$workspace_dir" TARGET_ARCH=qualcommax CURL_LOG="$curl_log" \
...
test -f "$openwrt_dir/files/usr/bin/easytier-web"
