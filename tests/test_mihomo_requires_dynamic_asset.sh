...
if PATH="$fake_bin:$PATH" OPENWRT_DIR="$openwrt_dir" WORKSPACE_DIR="$workspace_dir" TARGET_ARCH=qualcommax \
...
grep -q "No release asset matched mihomo-linux-arm64.*smart" "$log_file"
